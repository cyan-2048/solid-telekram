...
'auth.sentCodePaymentRequired':function(w,v){w.uint(3767884348);w.string(h(v,'storeProduct'));w.string(h(v,'phoneCodeHash'));w.string(h(v,'supportEmailAddress'));w.string(h(v,'supportEmailSubject'));w.string(h(v,'currency'));w.long(h(v,'amount'));},
...
'poll':function(w,v){w.uint(3091356649);w.long(h(v,'id'));var flags=0;if(v.closed===true)flags|=1;if(v.publicVoters===true)flags|=2;if(v.multipleChoice===true)flags|=4;if(v.quiz===true)flags|=8;var _closePeriod=v.closePeriod!==undefined;if(_closePeriod)flags|=16;var _closeDate=v.closeDate!==undefined;if(_closeDate)flags|=32;if(v.openAnswers===true)flags|=64;if(v.revotingDisabled===true)flags|=128;if(v.shuffleAnswers===true)flags|=256;if(v.hideResultsUntilClose===true)flags|=512;if(v.creator===true)flags|=1024;w.uint(flags);w.object(h(v,'question'));w.vector(w.object,h(v,'answers'));if(_closePeriod)w.int(v.closePeriod);if(_closeDate)w.int(v.closeDate);w.long(h(v,'hash'));},
...
'messages.composeMessageWithAI':function(w,v){w.uint(4248988414);var flags=0;if(v.proofread===true)flags|=1;var _translateToLang=v.translateToLang!==undefined;if(_translateToLang)flags|=2;var _changeTone=v.changeTone!==undefined;if(_changeTone)flags|=4;if(v.emojify===true)flags|=8;w.uint(flags);w.object(h(v,'text'));if(_translateToLang)w.string(v.translateToLang);if(_changeTone)w.string(v.changeTone);},
