...
'updateEphemeralBotCallbackQuery':function(w,v){w.uint(2604140386);w.long(h(v,'queryId'));w.int53(h(v,'userId'));w.object(h(v,'peer'));w.int(h(v,'msgId'));w.bytes(h(v,'data'));w.object(h(v,'message'));},
...
'ephemeralMessage':function(w,v){w.uint(3653688346);var flags=0;if(v.out===true)flags|=1;var _topMsgId=v.topMsgId!==undefined;if(_topMsgId)flags|=2;var _entities=v.entities&&v.entities.length;if(_entities)flags|=4;var _media=v.media!==undefined;if(_media)flags|=8;var _replyMarkup=v.replyMarkup!==undefined;if(_replyMarkup)flags|=16;var _replyTo=v.replyTo!==undefined;if(_replyTo)flags|=64;w.uint(flags);w.int(h(v,'id'));w.object(h(v,'fromId'));w.object(h(v,'peerId'));w.int53(h(v,'receiverId'));if(_topMsgId)w.int(v.topMsgId);w.int(h(v,'date'));w.string(h(v,'message'));if(_entities)w.vector(w.object,v.entities);if(_media)w.object(v.media);if(_replyMarkup)w.object(v.replyMarkup);if(_replyTo)w.object(v.replyTo);},
...
'ephemeral.sendMessage':function(w,v){w.uint(1758187679);var flags=0;var _queryId=v.queryId!==undefined;if(_queryId)flags|=1;var _entities=v.entities&&v.entities.length;if(_entities)flags|=2;var _media=v.media!==undefined;if(_media)flags|=4;var _replyMarkup=v.replyMarkup!==undefined;if(_replyMarkup)flags|=8;var _richMessage=v.richMessage!==undefined;if(_richMessage)flags|=16;var _replyTo=v.replyTo!==undefined;if(_replyTo)flags|=32;w.uint(flags);w.object(h(v,'peer'));w.object(h(v,'receiverId'));if(_queryId)w.long(v.queryId);w.string(h(v,'message'));if(_entities)w.vector(w.object,v.entities);if(_media)w.object(v.media);if(_replyMarkup)w.object(v.replyMarkup);if(_richMessage)w.object(v.richMessage);w.long(h(v,'randomId'));if(_replyTo)w.object(v.replyTo);},
'ephemeral.deleteMessage':function(w,v){w.uint(2747323665);w.object(h(v,'peer'));w.object(h(v,'receiverId'));w.int(h(v,'id'));},
...
'ephemeral.editMessage':function(w,v){w.uint(334647534);var flags=0;var _message=v.message!==undefined;if(_message)flags|=1;var _entities=v.entities&&v.entities.length;if(_entities)flags|=2;var _replyMarkup=v.replyMarkup!==undefined;if(_replyMarkup)flags|=4;var _media=v.media!==undefined;if(_media)flags|=8;w.uint(flags);w.object(h(v,'peer'));w.object(h(v,'receiverId'));w.int(h(v,'id'));if(_message)w.string(v.message);if(_media)w.object(v.media);if(_entities)w.vector(w.object,v.entities);if(_replyMarkup)w.object(v.replyMarkup);},
