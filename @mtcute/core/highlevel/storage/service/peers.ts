...
export class PeersService extends BaseService {
	...
	async store(peer: tl.TypeUser | tl.TypeChat): Promise<void> {
		...
		if (cached && this.options.updatesWriteInterval !== 0) {
			const oldAccessHash = (cached.peer as Extract<tl.TypeInputPeer, { accessHash?: unknown }>)
				.accessHash;

			const fixLong = (long?: Long) => (long ? new Long(long.low, long.high, long.unsigned) : long);

			if (fixLong(oldAccessHash)?.eq(accessHash)) {
