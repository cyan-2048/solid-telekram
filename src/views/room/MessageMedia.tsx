...
import { Download, downloadFile } from "@/lib/storage";
...
function StickerThumbnail() {
  ...
  const thumbnail = createMemo(() =>
    (media() as Sticker).getThumbnail(Thumbnail.THUMB_OUTLINE),
  );
        ...
        switch (type) {
          ...
          case "url": {
            const result = await callback(
              (url = URL.createObjectURL(download.result)),
            );
            if (result) url = result;
...
export function StickerMedia(props: FocusableMediaProps) {
  ...
  const [rlottieCanvasRef, setRlottieCanvasRef] =
    createSignal<null | HTMLCanvasElement>(null);
  ...
  const [rLottieFirstFrame, setRlottieFirstFrame] =
    createSignal<null | ImageData>(null);
  ...
  createEffect(() => {
    ...
    async function tick_cb(timestamp: number) {
      ...
      if (frameIndex !== currentFrame) {
        ...
        const imageData =
          cachedFrames[currentFrame] || (await requestFrame(id, currentFrame));
  ...
  createEffect(async () => {
    ...
    const clampedBuffer = await rlottie.requestFrame(
      sticker.uniqueFileId,
      0,
      128,
      128,
    );
  ...
  return (
    <div class={styles.sticker}>
      ...
      >
        <Show when={!focused()}>
          <Show
            when={rLottieFirstFrame()}
            fallback={<StickerRlottieFirstFrameWebp />}
          >
    ...
    if (thumb && "byteLength" in thumb.location) {
      setThumb(
        (url = URL.createObjectURL(
          new Blob([thumb.location as Uint8Array<ArrayBuffer>]),
        )),
      );
    }
...
function VideoMedia(props: FocusableMediaProps) {
  ...
  createEffect(() => {
    ...
    if (media.fileSize > 5242880) {
      console.error(
        "SKIPPING DOWNLOAD BECAUSE FILE SIZE TOO BIG",
        niceBytes(fileSize || 0),
      );
  ...
  createEffect(() => {
    ...
    if (audioIsPlaying) {
      setSoftkeys(
        "Rewind",
        "PAUSE",
        (untrack(audioSpeed) == 2 ? 1 : untrack(audioSpeed) + 0.5) + "x",
      );
    ...
    <>
            ...
            <Index each={waveform()}>
              {(num, index) => (
                <div
                  style={{
                    height:
                      Math.min(100, Math.max(12, (num() / 30) * 100)) + "%",
                    background:
                      waveformIndex() != 0 && index <= waveformIndex()
                        ? "var(--accent)"
                        : undefined,
                  }}
                  class={styles.wave}
      ...
      <audio
        ...
        onTimeUpdate={(e) => {
          ...
          setWaveformIndex(
            Math.floor(
              (e.currentTarget.currentTime / e.currentTarget.duration) * 31,
            ),
          );
        }}
        onEnded={() => {
...
function DocumentMedia(props: FocusableMediaProps) {
  ...
  const iconUrl = createMemo(
    () =>
      "https://file-icons.cyan-2048.workers.dev/" +
      (media() as Document).mimeType,
  );
...
function MusicMedia(props: FocusableMediaProps) {
  ...
  const iconUrl = createMemo(
    () =>
      "https://file-icons.cyan-2048.workers.dev/" + (media() as Audio).mimeType,
  );
