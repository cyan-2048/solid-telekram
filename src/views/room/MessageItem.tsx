...
import { UIMessageUploading } from "@/ui/UIDialog";
...
export function decideDateSepatator(
...
export function MessageProvider(props: {
  ...
  const [reply, setReply] = createSignal<UIMessage | undefined | 0 | 1>(
    undefined,
  );
  ...
  const showChecks = createMemo(
    () => props.$.isOutgoing && !(entities().entities || entities().text),
  );

  const showTail = createMemo(() => decideTail(props.before, props.$));
  const showUsername = createMemo(() =>
    decideShowUsername(props.before, props.$),
  );
  ...
  const showDateSeparator = createMemo(() =>
    decideDateSepatator(props.before?.date, props.$.date),
  );
        ...
        <Show when={canDeleteForMe(dialog())}>
          <OptionsItem
            ...
            on:sn-navigatefailed={(e) => {
              ...
              if (
                direction == "up" &&
                canDeleteForEverone(rawMessage(), dialog())
              ) {
...
function MessageAction() {
  ...
  onCleanup(() => {
    ...
    if (
      $view.get() == "room" &&
      actEl &&
      (actEl.classList.contains(styles.action_message) ||
        actEl.classList.contains(styles.message))
    ) {
  ...
  return (
    <Show
      when={
        actionType() != "history_cleared" && actionType() != "contact_joined"
      }
    >
      <ActionMessage
        ref={(el) => {
            ...
            if (e.key == "SoftRight") {
              if (mediaType() == "voice") {
                ...
                setSoftkeys(
                  null,
                  null,
                  (audioSpeed() == 2 ? "1" : audioSpeed() + 0.5) + "x",
                );
              } else {
                EE.emit("audio_stop");
              }
          ...
          >
            ...
            <div class={styles.text_container}>
              <div class={styles.text}>
                {
                  <Markdown
                    entities={joinTextWithEntities([props.upload.text])}
                  />
                }
                <span
...
export function MessageItemInner(props: {
  ...
  const [isOverflowing, setOverflowing] = createSignal(false);
  const [overflowDivRef, setOverflowDivRef] =
    createSignal<null | HTMLDivElement>(null);
        ...
        fallback={
          <>
            <Show when={showUsername()}>
              <UsernameContainer peer={message().sender.raw}>
                {message().sender.displayName}
              </UsernameContainer>
            </Show>
            <div
              ref={setOverflowDivRef}
              class={isExpanded() ? styles.not_overflowing : styles.overflowing}
            >
              <Switch>
                ...
                <Match when={reply() === 1}>
                  <Show when={message().getQuote()}>
                    {(quote) => <QuoteReplyMessage {...quote()} />}
                  </Show>
                </Match>
                <Match when={reply()}>
              ...
              />
              <Show
                when={!isSticker() && (entities().entities || entities().text)}
              >
                <div class={styles.text_container}>
                  <div class={styles.text}>
                    {
