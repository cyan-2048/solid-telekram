...
import { EmojiCategory, getEmojiPage, getLastEmojiPage } from "@/workers";
import { useStore } from "@nanostores/solid";
import { $emojiHistory } from "@/stores";
	...
	return (
		<div
			...
			onFocus={() => {
				...
				if (cloudphone) setSoftkeys("Cancel", "", "");
				else setSoftkeys("tg:up", "", "tg:down");
			}}
...
function EmojiItem(props: { onSelect: (e: string) => void; emoji: string; num: string }) {
	...
	createEffect(() => {
		...
		if (focused()) {
			untrack(() => {
				if (cloudphone) setSoftkeys("Cancel", "SELECT", "");
				else setSoftkeys("tg:up", emoji ? "SELECT" : "", "tg:down");
			});
		}
	});
	...
	return (
		<div
			...
			on:sn-enter-down={() => {
				if (props.emoji) props.onSelect(props.emoji);
			}}
			class={styles.emojiItem}
		>
			<div class={styles.emoji}>
				<Show when={props.emoji}>{(str) => <MarkdownText text={str()} />}</Show>
			</div>
...
export default function EmojiPicker(props: { onSelect: (e: null | string) => void }) {
	...
	const emojiHistory = useStore($emojiHistory);
	...
	function onSelect(emoji: string | null) {
		props.onSelect(emoji);
		if (emoji) {
			const newHistory = $emojiHistory.get().filter((a) => a !== emoji);
			newHistory.unshift(emoji);
			$emojiHistory.set(newHistory.slice(0, 12));
		}
	}
				...
				>
					<EmojiCategoryItem setSelected={setSelected} selected={selected()} category={null} />
					{
						...
						[
							...
							<EmojiCategoryItem setSelected={setSelected} selected={selected()} category={category} />
						))
					}
				</div>
				<div
