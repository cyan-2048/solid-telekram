 ...
 */
declare const LyricsContentType: {
    other: number;
    lyrics: number;
    text: number;
...
type LyricsContentType = typeof LyricsContentType[keyof typeof LyricsContentType];
declare const TimestampFormat: {
    notSynchronized: number;
    mpegFrameNumber: number;
    milliseconds: number;
};
type TimestampFormat = typeof TimestampFormat[keyof typeof TimestampFormat];
...
type FormatId = 'container' | 'duration' | 'bitrate' | 'sampleRate' | 'bitsPerSample' | 'codec' | 'tool' | 'codecProfile' | 'lossless' | 'numberOfChannels' | 'numberOfSamples' | 'audioMD5' | 'chapters' | 'modificationTime' | 'creationTime' | 'trackPeakLevel' | 'trackGain' | 'albumGain' | 'hasAudio' | 'hasVideo';
 ...
 * @returns Metadata
 */
declare function parseBlob(blob: Blob, options?: IOptions): Promise<IAudioMetadata>;
